...
int OS_FileOpen_hook(int a1, int a2, const char *a3, int a4)
{
  int v8; // r0
  int v9; // r4
  char v11[256]; // [sp+0h] [bp-120h] BYREF

  memset(v11, 0, sizeof(v11));
  if ( !strncmp(a3, "scache.txt", 0xAu)
    || !strncmp(a3, "scache_small_low.txt", 0x14u)
    || !strncmp(a3, "scache_small.txt", 0x10u) )
  {
    return 0;
  }
...
void MobileMenu__Render_hook(uintptr_t thiz)
{
	...
	CFont::SetScaleXY(1.0, 1.0);
	CFont::SetCentreSize(0);
	if(pGUI)CFont::PrintString(pGUI->ScaleX(10), pGUI->ScaleY(10), cryptor::create("v1.0", 4).decrypt());
}
